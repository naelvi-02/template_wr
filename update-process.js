...
const processFilesStart = code.indexOf('newEntries.forEach(async (entry) => {');
const processFilesEnd = code.indexOf('    });\n  }, [activeId]);');

if (processFilesStart === -1 || processFilesEnd === -1) {
  console.log("Could not find processFiles loop");
} else {
  const oldLoop = code.substring(processFilesStart, processFilesEnd + 7);
  const newLoop = `const processQueue = async () => {
